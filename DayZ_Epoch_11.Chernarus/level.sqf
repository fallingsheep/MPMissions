...
// Wait for the player got his login data
...
//start level system
[] execVM "scripts\levels.sqf";

//Level menu stuff for fn_selfactions
if (LevelMenu) then {
    if (s_player_openlevel < 0) then {
        s_player_openlevel = player addaction[("<t color=""#9ABCD9"">" + ("Open Level Menu") +"</t>"),"scripts\menu.sqf","",0,false,true,"", ""];
    };
} else {
